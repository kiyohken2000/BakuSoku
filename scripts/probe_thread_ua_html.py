...
def main():
    paths = {
        "rw1_p1": f"/thr_res/acode={acode}/ctgid={ctgid}/bid={bid}/tid={tid}/tp=1/rw=1/",
        "rw1_p2": f"/thr_res/acode={acode}/ctgid={ctgid}/bid={bid}/tid={tid}/p=2/tp=1/rw=1/",
    ...
    for label, path in paths.items():
        for ua_label, ua in [("mobile", UA_MOBILE), ("pc", UA_PC)]:
            html = fetch(path, ua)
            fname = f"{label}_{ua_label}.html"
            save(fname, html)
            s = summarize_html(html)
            rows.append((label, ua_label, s))

    for label, ua_label, s in rows:
        title = safe_str(s['title'])
        print(f"{label} / {ua_label}: title={title!r} res_blocks={s['res_blocks']} has_res0={s['has_res0']} has_paging={s['has_paging']} ids={s['ids']}")
